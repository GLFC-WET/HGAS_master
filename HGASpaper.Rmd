...
if(!file.exists("out_workspaces/LMER_numfish_models.RDS")){
  ...
  mutate(predicted = exp(predicted_value)) %>%
      ...
      out <- data.frame(Cor = round(temp$mod$estimate, 2),
                     ...
                     confint =  ifelse(class(temp$mod2) != "try-error", str_c(round(confint(temp$mod2, "predicted", .95),2), collapse = "-"), NA))
      }
  ...
  LMER_fish_data <- LMER_fish_data %>%
    ...
    group_by() %>%
    dplyr::select(SPECIES_NAME, CONTAMINANT, num_lakes, num_fish, rep, Cor, p)
...
cor_sums <- models_accuracy_by_N %>% rbind(mean_mods) %>%
  ...
  mutate(CONTAMINANT = str_extract(set, "^Hg|^As"), 
         ...
         COR2.text = ifelse(!is.na(Cor), paste0("r = ", Cor, "\np ",ifelse(cor.p < 0.05, "< 0.05", paste0("= ", cor.p))), "Not enough data")) %>% 
...
lm_sums <- models_accuracy_by_N %>% rbind(mean_mods) %>%
  ...
  mutate(CONTAMINANT = str_extract(set, "^Hg|^As"), 
         SPECIES_NAME = str_extract(set, "Lake Trout|Walleye|Northern Pike"),
         response = factor(str_extract(set, "SER|median|boot_REML|REML|MCMC|INLA_ll|INLA"), levels = c("SER", "median", "REML", "boot_REML", "MCMC", "INLA", "INLA_ll")),
         R2.text = ifelse(!is.na(confint), paste0(confint), "Not enough data")) %>% 
  arrange(SPECIES_NAME, response, CONTAMINANT) %>%
  mutate(plotorder =paste(SPECIES_NAME, response, CONTAMINANT), 
         plotorder = factor(plotorder, levels = unique(plotorder))) %>% 
...
plot.corrs <- ggplot(lm_sums %>% filter(CONTAMINANT == "As") %>%
  pivot_longer(cols = c(SPECIES_NAME, response, R2.text, COR2.text), names_to = "column", values_to = "text") %>% 
    mutate(column = gsub("response", "Model", column), 
           ...
           column = gsub("R2.text", "Slope", column)) %>% 
    mutate(column = factor(as.character(column), levels = c("Fish Species", "Model", "Correlation", "Slope"))), aes(column, plotorder)) + 
...
We found that mixed effects approaches performed better than SER for representing the trends in the data, as represented by improved correlations. The mixed models also enabled predictions of values for low sample sizes with only slight accuracy reductions in the As models or marked accuracy improvements in the Hg models, compared to SER ([As] Figure \@ref(fig:pred-resid-table-fig-As), [Hg] Figure \@ref(fig:pred-resid-table-fig-Hg), Figure S\@ref(fig:pred-1000g-rem)). The REML, MCMC and INLA values were generally good at representing the trends in the samples, as they were more strongly correlated to actual concentrations (r ranging from `r text.info$cor[text.info$CONTAMINANT == "As" & text.info$response == "MEM"]` in [As] and `r text.info$cor[text.info$CONTAMINANT == "Hg" & text.info$response == "MEM"]` in [Hg]) than predictions from SER models (r ranging from  `r text.info$cor[text.info$CONTAMINANT == "As" & text.info$response == "SER"]` in [As] and `r text.info$cor[text.info$CONTAMINANT == "Hg" & text.info$response == "SER"]` in [Hg]) (Figure \@ref(fig:pred-resid-table-fig-As), \@ref(fig:pred-resid-table-fig-Hg), Figure S\@ref(fig:pred-1000g-rem)). Prediction accuracy of for [Hg] with the REML, MCMC and INLA (slopes deviating `r text.info$slp[text.info$CONTAMINANT == "Hg" & text.info$response == "MEM"]` from 1) approaches was better than SER (slopes deviating `r text.info$slp[text.info$CONTAMINANT == "Hg" & text.info$response == "SER"]` from 1), but [As] prediction in REML, MCMC and INLA (slopes deviating `r text.info$slp[text.info$CONTAMINANT == "As" & text.info$response == "MEM"]` from 1) was slightly poorer, compared to SER (slopes deviating `r text.info$slp[text.info$CONTAMINANT == "As" & text.info$response == "SER"]` from 1). The reduced prediction accuracy in [As] was small, being generally with slopes within 0.05 of SER values. The reduction in [As] mixed model performance may have been due to the lower overall dataset sizes for the [As] models, and possibly, also influenced by the assumptions of relationships between weight and [As] made in the structure of the mixed models. As [As] was more accurately predicted using medians for Northern Pike and Walleye than any of the linear modelling approaches, that appears to be the case for those two species. Lake Trout might be an exception from this, as it could be prone to slightly more bioaccumulation of As due to it's slow growth rate. The increased accuracy of [Hg] models can be attributed to the elimination of influential residuals that resulted from the magnification of error after back-transformation of the log-transformed SER predictions. Also, the residuals between the back-transformed predicted values and measured concentrations for the different mixed modeling approaches were generally similar or smaller than those of the SER approach for both [As] (Figure \@ref(fig:pred-resid-table-fig-As)) and [Hg] (Figure \@ref(fig:pred-resid-table-fig-Hg)) models. Interestingly, attempts to improve the REML approach with bootstrapping did not increase accuracy of predictions, even resulting in decreased correlation strengths and larger deviations of slopes from a one-to-one relationship in some cases (specifically for the [Hg] Northern Pike and Lake Trout models (Figure  \@ref(fig:pred-resid-table-fig-Hg)), and the [As] Walleye model (Figure \@ref(fig:pred-resid-table-fig-As)) ([As]). This is likely because the bootstrapping process overfit the models, as demonstrated by decreased RMSE (Figure \@ref(fig:RMSE-N-WE)), which reduced the ability of the models to produce generalized predictions. 
...
The simulated samplings also revealed that REML models perform better with more balanced sampling designs than INLA models. Generally, in the random samples, which had an equal number of fish selected for each lake sample event included in the sample, REML performed better than INLA models (Figure S\@ref(fig:LTas-prednum), S\@ref(fig:NPas-prednum), S\@ref(fig:WEas-prednum), S\@ref(fig:LThg-prednum), S\@ref(fig:NPhg-prednum), S\@ref(fig:WEhg-prednum)). This contrasted with the results from the assessment of the test dataset, where REML and INLA models performed comparably in terms of correlation scores (Figure \@ref(fig:pred-resid-table-fig-As), Figure \@ref(fig:pred-resid-table-fig-Hg)). As the REML models had slightly lower performance in models created from the test dataset (Figure \@ref(fig:pred-resid-table-fig-As), Figure \@ref(fig:pred-resid-table-fig-Hg)), this suggests that the REML models are more sensitive to sampling imbalance than INLA models. These results suggest that INLA would be preferred over REML in scenarios, for example, where researchers fail to collect sufficient numbers or size ranges of fish for each sample event - thus resulting in imbalanced datasets. In scenarios where there are balanced sample collections and a high number of sample events, the easier to implement REML models are likely to perform comparably to INLA. 
