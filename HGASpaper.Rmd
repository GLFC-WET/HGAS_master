...
- Brian W. Kielstra, Ecometrix and Great Lakes Forestry Centre, Natural Resources Canada, bkielstra@ecometrix.ca
...
Our evaluations of different sampling scenarios of varied numbers of lakes and fish numbers per sample event (see Methods: Simulation of different sampling scenarios) confirmed that INLA and REML were effective at creating predictions for lakes with low sample numbers, but both required a large number of lakes. Northern Pike, Lake Trout and Walleye [Hg] prediction was possible in simulations where 4 fish were used, but these predictions required more than 100 lakes (Figure S\@ref(fig:LThg-predslp), S\@ref(fig:NPhg-predslp), S\@ref(fig:WEhg-predslp),  S\@ref(fig:LThg-prednum), S\@ref(fig:NPhg-prednum)). For example, prediction of Lake Trout [Hg] required models with over 200 fish to predict using four fish per lake to produce predictions that were accurate (i.e., had higher correlation and approached 1:1 linear relationship with measured values with a zero intercept) (Figure S\@ref(fig:LThg-predslp)). Arsenic results were more varied, with much poorer linear associations between predicted and measured values, and poorer correlation strengths than [Hg]. However, none of the [As] datasets had very large number of lakes, which limited this line of investigation. These results show that prediction of contaminant concentrations with few fish per sample event is possible when larger, existing datasets are leveraged. We suggest that creating predictive models that leverage the power of existing large datasets with hundreds of lakes, such as the MECP database used in this study, would be the most effective way to employ mixed-model standardization approaches.


The simulated samplings also revealed that REML models perform better with more balanced sampling designs than INLA models. Generally, in the random samples, which had an equal number of fish selected for each lake sample event included in the sample, REML performed better than INLA models in correlations as well as prediction accuracy, as represented by the linear relationship between predicted and measured values approaching a 1:1 relationship (Figure S\@ref(fig:LTas-predslp), S\@ref(fig:NPas-predslp), S\@ref(fig:WEas-predslp), S\@ref(fig:LThg-predslp), S\@ref(fig:NPhg-predslp), S\@ref(fig:WEhg-predslp), S\@ref(fig:LTas-prednum), S\@ref(fig:NPas-prednum), S\@ref(fig:WEas-prednum), S\@ref(fig:LThg-prednum), S\@ref(fig:NPhg-prednum), S\@ref(fig:WEhg-prednum)). This contrasted with the results from the assessment of the full test dataset, where REML had greater or similar slope deviations from 1 and similar correlation scores compared to INLA models (Figure \@ref(fig:pred-resid-table-fig-As), Figure \@ref(fig:pred-resid-table-fig-Hg)). As the REML models had slightly lower performance in models created from the test dataset (Figure \@ref(fig:pred-resid-table-fig-As), \@ref(fig:pred-resid-table-fig-Hg)), this suggests that the REML models are more sensitive to sampling imbalance than INLA models. These results suggest that INLA would be preferred over REML in scenarios, for example, where researchers fail to collect sufficient numbers or size ranges of fish for each sample event - thus resulting in imbalanced datasets. In scenarios where there are balanced sample collections and a high number of sample events, the easier to implement REML models are likely to perform comparably to INLA. Both INLA and REML were biased towards slight underestimation concentrations, as evidenced by accuracy models with slopes greater than one (Figure \@ref(fig:pred-resid-table-fig-As), S\@ref(fig:WEas-predslp), S\@ref(fig:LThg-predslp), S\@ref(fig:NPhg-predslp), S\@ref(fig:WEhg-predslp)).  


The INLA models developed from the simulated samples may have had a more consistent predictive behaviour with respect to increasing the size of the overall dataset (i.e., have improved prediction when either the number of fish per sample event, or number of lakes increased) than REML models. This pattern was particularly apparent in the Walleye [Hg] dataset, where increases in numbers of fish didn’t result in increased linear associations or correlations of predicted and measured values for models with 2-75 lakes (Figure S\@ref(fig:WEhg-predslp), S\@ref(fig:WEhg-prednum))), while INLA models generally had stronger linear associations and increased correlation with increases in number of fish for all sizes of lakes (Figure S\@ref(fig:WEhg-predslp), S\@ref(fig:WEhg-prednum)). This characteristic of INLA models means any addition of data to the training dataset will benefit the overall INLA model, whereas REML models are only guaranteed to have increased performance with an increasing number of lakes and typically had good performance in models with 10-20 fish. 
...
nfishes_lm <- rbind(LMER_fish_data %>% 
                   mutate(Anal. = "REML"), 
                 INLA_fish_data %>% 
                   mutate(Anal. = "INLA")) %>% 
...
lm_num_plot <- function(fish, contaminant){
  plot_slp <- ggplot(nfishes_lm %>% filter(SPECIES_NAME == fish, CONTAMINANT == contaminant), aes(as.numeric(as.character(num_fish)), av.Slp, color = Anal., fill = Anal.)) + 
  geom_line() + 
    geom_point()+
  geom_ribbon(aes(ymin = minSlp, ymax = maxSlp), alpha = 0.5) + facet_grid(num_lakes~., scales = "free_x") + 
  xlab("Number of fish per lake") + ylab(paste0("Slope of predicted and measured [",contaminant,"]")) + coord_cartesian(ylim = c(0, 2))

  plot_Int <- ggplot(nfishes_lm %>% filter(SPECIES_NAME == fish, CONTAMINANT == contaminant), aes(as.numeric(as.character(num_fish)), av.Int, color = Anal., fill = Anal.)) + 
  geom_line() + 
  geom_point() +
  geom_ribbon(aes(ymin = minInt, ymax = maxInt), alpha = 0.5) +
  facet_grid(num_lakes~., scales = "free_x") + 
  xlab("Number of fish per lake") + ylab(paste0("Intercept of predicted and measured [",contaminant,"]")) + coord_cartesian(ylim = c(-1, 1))

  plot_0 <- as_ggplot(text_grob(paste(fish, contaminant), size = 15)) 

  ggarrange(plot_0 , ggarrange(plot_slp, plot_Int, ncol = 2, common.legend = T, legend = "bottom"), nrow = 2, heights = c(0.05, 0.95))
}

plotLTHg <- lm_num_plot("Lake Trout", "Hg")
plotLTAs <- lm_num_plot("Lake Trout", "As")

plotNPHg <- lm_num_plot("Northern Pike", "Hg")
plotNPAs <- lm_num_plot("Northern Pike", "As")

plotWEHg <- lm_num_plot("Walleye", "Hg")
plotWEAs <- lm_num_plot("Walleye", "As")
```


```{r LTas-predslp, fig.width=8.5, fig.height=10, fig.cap="Range of slopes and intercepts for simulated samplings of Lake Trout [As] INLA and REML predictions from different lake and fish number combinations. Maximum and minimum slopes and intercepts from the 95% confidence intervals for each fish number (x-axis) and lake number (vertical panels) combination are represented by a shaded area and the median slope or intercept for each simulations are represented as a central line with points. INLA and REML results are distinguished by colour."}
plotLTAs
```
...
```{r WEas-predslp, fig.width=8.5, fig.height=11, fig.cap="Range of slopes for simulated samplings of Walleye [As] INLA and REML predictions from different lake and fish number combinations. Maximum and minimum slopes and intercepts from the 95% confidence intervals for each fish number (x-axis) and lake number (vertical panels) combination are represented by a shaded area and the median slope or intercept for each simulations are represented as a central line with points. INLA and REML results are distinguished by colour."}
plotWEAs
```

```{r LThg-predslp, fig.width=8, fig.height=15, fig.cap="Range of slopes for simulated samplings of Lake Trout [Hg] INLA and REML predictions from different lake and fish number combinations. Maximum and minimum slopes and intercepts from the 95% confidence intervals for each fish number (x-axis) and lake number (vertical panels) combination are represented by a shaded area and the median slope or intercept for each simulations are represented as a central line with points. INLA and REML results are distinguished by colour."}
plotLTHg
```
...
```{r WEhg-predslp, fig.width=8, fig.height=15, fig.cap="Range of slopes for simulated samplings of Walleye [Hg]  INLA and REML predictions from different lake and fish number combinations. Maximum and minimum slopes and intercepts from the 95% confidence intervals for each fish number (x-axis) and lake number (vertical panels) combination are represented by a shaded area and the median slope or intercept for each simulations are represented as a central line with points. INLA and REML results are distinguished by colour."}
plotWEHg
```
