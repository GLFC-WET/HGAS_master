...
names(cust_cols) <- c("SER", "LMER", "boot_LMER", "MCMC", "INLA")
```

# Abstract

Size and age-standardization of contaminant concentrations is a common practice for bioaccumulative contaminants like mercury (Hg). Sampling event regressions (SERs) are frequently used to estimate size-adjusted concentrations of these contaminants in fish populations prior to making comparisons or further modeling. However, this approach can be limited by sample size constraints within and across waterbodies. Herein, we compare mixed effect linear models (LMER), Bayesian inference via Markov Chain Monte Carlo (MCMC) and approximate Bayesian inference using integrated nested Laplace approximation (INLA) approaches that borrow strength from all available observations to estimate size-adjusted contaminant concentrations in individual fish populations. We found that RSTAN and INLA produced comparable predictions to SER approaches, with increased accuracy and positive bias for estimation errors. LMER models conversely, had less accuracy and error trended towards underestimating values. The INLA and RSTAN approach allowed for the estimation of values for lake populations with less than 5 fish, and which lacked the range of fish sizes required for SER estimates. INLA is the more suitable approach, with fast speeds and better overall accuracy than RSTAN models. We recommend INLA as a reliable means of estimating size-adjusted contaminant concentration in fish populations, particularly when datasets include some lake populations where sample sizes are limited.
...
# Synopsis
 ...
 !!! TOC art will go here !!!

# Introduction


Contaminant research often need to account for the influence of age and size on contaminant concentrations for the analysis of environmental drivers of potentially harmful compounds. Bioaccumulative contaminants - which are incorporated into tissues faster than they can be excreted - are of particular concern because they can reach elevated concentrations in organisms, even in remote environments distant from primary sources of pollution.  As a result, older organisms generally have higher concentrations of these contaminants. Contaminant age standardization is a method that is commonly applied in studies of aquatic organisms [@izagirre2014;@belpaire2007;@lescord2019;@kluke2022], but also have applications for studying polar bear [@verreault2004], moose [@danielsson2009] and even human populations [@pal2003]. Fish populations can serve as an ideal model system to test the effectiveness of size standardization methods. In fish, age is often correlated with metrics of fish body size (i.e., weight and length), which enables size-standardization to be used in place of age-standardization. Additionally, fish are regularly monitored as consumption of wild-caught fish, while abundant with healthy fats and protein, is considered a potentially detrimental source of contaminants to the human body [@canada2019]. Monitoring programs rely on accurate estimates of the relationships between contaminant concentration and fish size (concentration-size relationships) to inform fish consumption recommendations [@mecp2023;@bhavsar2011]. In addition, these relationships are also imperative for studies attempting to understand how land use, forest change, and natural variation affect contamination, to account for differences in concentrations related to fish age and size among studied ecosystems [@kidd2012;@lescord2020]. This means these datasets are collected to enable size-standardization for a diverse set of populations. The commonly used approach of accounting for this concentration-size relationship is to regress contaminant concentrations against metrics of body size within a fish population (usually a unique species-lake combination for a given year) and use the resulting linear equation, or an ANCOVA model to estimate the concentration at a chosen size (e.g., 500 g or 1kg) [@kidd2012;@lescord2019;@sumner2020;@sonesten2003]; herein we refer to this approach as Sampling Event Regressions (SERs).


Size-adjustment through SER requires consideration at the sampling level, because of the range in sizes required, and not every dataset is suitable. Some contaminants are widely acknowledged as requiring size adjustment, and have rich datasets that enable SER on many populations, while others may be limited by cost of analyses and additional sampling requirements. Mercury ([Hg]) is a highly bioaccumulative contaminant of global concern that is routinely monitored in fish. Mercury is well known to accumulate in the muscle tissue of fish, because of its chemical speciation and inter-organ transport [@peng2016]. Many studies have found strong positive correlations between Hg concentrations and body size in both freshwater [@johnston2022] and marine [@al-sulaiti2022] environments and, thus, size-adjustments of Hg concentrations prior to making comparisons across environmental gradients are common [@lescord2020;@sumner2020;@eagles-smith2016;@eckley2023;@negrazis2022], and datasets as a result are suited to this approach. Other contaminants, like arsenic, are less bioaccumulative in fish, showing mixed and weaker relationships with metrics of body size [@lescord2020;@kluke2022]. Arsenic also represents a contaminant that is more costly to analyze, meaning there may be less data covering a wide range of sizes. Arsenic is a good example of a weakly bioaccumulative contaminant that poses significant concern. Depending on its chemical speciation, arsenic can be a harmful carcinogenic contaminant, but it is less widely distributed and often originates from a localized point source of contamination or natural abundance. It is also less routinely monitored and researched when compared to [Hg] in fish[@lescord2020; @ontario2023], which results in less data to be available for size-adjustment models [@ponton2022]. Not accounting for size in population-level comparisons of contaminants could mean that size-related variation could be incorrectly attributed to other attributes of a sample, possibly weakening predictions and limiting our understanding of environmental cycling and bioaccumulation patterns. Many emerging contaminants of concern face similar issues of unknown or low bioaccumulative potential, and a lack of data for effective size-adjustment using SER.


The common standard of size-adjustment (SER) requires higher sample numbers and a broad distribution of sizes of fish, which may not be attainable for all monitoring surveys or studies. This lack of appropriate sample coverage can result in exclusion of lakes from studies. In general, research and monitoring programs sample waterbodies for fish seek to attain minimum sample size (e.g., 10-20 individuals/species) across as broad of a size range as possible to ensure a reasonable estimate of the population [@johnston2022; @weber2021; @st-onge2000]. However, opportunistic sampling and analytical expense can limit the number of fish caught, the size range represented, or the distribution of fish sizes for a given population – all of which may lead to over/under estimating standardized concentrations when using an SER approach. Small sample sizes can be particularly limiting in assessments of temporal trends, and may lead researchers to pool samples over time periods, such as in Bhavsar *et al.,* [-@bhavsar2010]. Alternatively, deficient populations may be excluded altogether, sacrificing valuable data often from underrepresented waterbodies or fish species. While there are modelling approaches that can use information from across sampling events to allow estimates where SER cannot be performed, these have not been broadly adopted or validated for contaminant size-standardization. Mixed effects regression approaches have been used to overcome this limitation by incorporating size into a model [@depew2013;@lescord2020;@buck2019], but are not often employed for size-standardized estimates, and can result in effects that are difficult to interpret due do the co-variance of size distributions and other independent effects. Bayesian linear mixed effects models [@ffong2010] are also potential solution as they utilize all data across sampling structures (e.g., waterbodies) to inform predictions and allow for explicitly modeling variation of random effects variables. Bayesian models have the advantage of being able to update probabilities of predictions based on inference from all data instead just the observation itself (i.e., strength of relationships between other similar predictor combinations are able to influence the prediction of a particular point).These models can produce more accurate estimates of model parameters, and more probable predictions for individual points, but have a trade-off of requiring prior assumptions, which can be difficult to determine [@fong2010]. Different Bayesian linear mixed effects models have been implemented in R, including Bayesian inference via Markov Chain Monte Carlo (MCMC) approach in the (rstanarm) package [@R-rstanarm]. MCMC is a commonly applied Bayesian approach [@fong2010], but it has high computational requirements. Approximate Bayesian inference approaches that are much less computationally intensive than MCMC are also available, integrated Laplace approximation is one of these, and is available in the (INLA) R package [@R-INLA; @INLA2017a]


Herein, we compare the commonly applied SER as a method of producing size-standardized contaminant values to several mixed effect model (MEM) approaches to determine which approach can produce accurate size-estimates for populations that have insufficient data for SER: an easily implemented mixed effects regression (using the lmer package), a common Bayesian inference approach (MCMC) and approximate Bayesian inference that should achieve the accuracy of the standard Bayesian approach with less computational requirements (INLA). We evaluate these approaches using two contaminant measures representing two common scenarios, the highly monitored and bioaccumulative total [Hg] and the less frequently monitored and less bioaccumulative [As] in fish muscle tissue. These two different contaminants will be used to assess effectiveness of the modeling approaches for contaminants with different bioaccumulative potential and data availability. We hypothesize that mixed modelling approaches will provide comparable or more accurate predictions of contaminant concentrations to SER, and enable estimation in data-scarce populations. We expect linear mixed model (LMER) estimates to produce comparable results to SER, but have more error than Bayesian models, as LMER models are not as robust at estimating random effects. We also expect that Bayesian and approximate Bayesian approaches will have improved accuracy compared to more LMER, especially for predictions in lakes with low sample numbers. We expect that the approximate Bayesian model (INLA) will outperform a more traditional Bayesian (MCMC) model in terms of computation time, but have similar predictive accuracy. 
...
We obtained fish-level Hg and As data for inland waterbodies across Ontario from the Ontario Ministry of Environment, Conservation, and Parks (OMECP). Although individual fish sampling protocols varied, muscle tissue sent to OMECP is analyzed using standardized methods as part of the Fish Contaminants Monitoring Program [@ontario2023]. More specifically, total [Hg] were measured using cold vapor-flameless atomic absorption spectroscopy (CV-FAAS) following protocol OMECP-HGBIO-WS057 and total [As] was measured using Inductively Coupled Plasma Mass Spectrometry (ICP-MS) following method OMECP-BIOTA-E3461. We limited our analyses to inland lakes and three species of interest: *Salvelinus namaycush namaycush* (common Lake Trout, hereafter Lake Trout), *Esox lucius* (Northern Pike), and *Sander vitreus* (Walleye). These species were selected as they are all predatory fish, which, as the top of the food chain, are prone to bioaccumulating contaminants, and are some of the primary indicator species collected in the FMCP[@ontario2023]. These three species also represent distinct niches within lakes: while Lake Trout are generally restricted to profundal zones, Northern Pike generally prefer shallow habitats and Walleye have broader movement patterns. In total, the final dataset we used included [Hg] in `r fish_amounts$total_fish[fish_amounts$CONTAMINANT == "Hg"]` fish and [As] in `r fish_amounts$total_fish[fish_amounts$CONTAMINANT == "As"]` fish. The two contaminants were selected as [Hg] is known to bioaccumulate easily, and has rich monitoring datasets; Arsenic [As] was selected as it is an emerging contaminant of concern, with a more subtle bioaccumulation signal in fish.  

To compare the modelling approaches, we used two weight-standardization targets of 500g and 1000g, as these were well represented in the dataset\@ref(fig:weight-values), and were near the mid-point of weights represented in the dataset. These weights have been used in other studies for similar reasons [@lescord2020; @sumner2020;@lescord2019]. Fish from the targeted weight classes were randomly sampled to create a test dataset with ~50% of the fish within 400 - 600g and 900 - 1100g and a train dataset with the remaining data from other fish size classes, as well as the remaining ~50% of fish in the targeted size classes (Tables \@ref(tab:sampling-events-summary-train), \@ref(tab:sampling-events-summary-test)) 
...
All modelling and analyses were performed in R Version 4.2.3 [@R-base] on a Dell Latitude 5510 PC running Windows 10 Enterprise with a 1.70GHz, 2208 MHz 4 Core(s), 8 logical Processor, 16 GB of physical memory and 34.6 GB of virtual memory. All data transformations, summarizations and graphing were performed with the tidyverse, ggplot2 and ggpubr packages [@R-tidyverse; @R-ggplot2; @R-ggpubr].


### Sampling event regressions (SER)


For each contaminant-species-waterbody-year combination (i.e., a sampling event), we developed the usual log-log (log-contaminant (ug/g) by log-weight (g)) regression models for those combinations with at least 5 sampled individuals, and only for sampling events where there were values above and below at least one of our targeted weights (i.e., <500 to >500 and/or <1000 to >1000) for standardization. This number of fish is lower than usually employed with this approach, due to low sample sizes in the Arsenic dataset, and we wanted to compare performance of these approaches, with both datasets, when employed with unusually limited sample sizes. Our basic size-range standard was used in the interest of efficiency, but is a more lenient approach than is typically employed by researchers (who often evaluate each model independently, with different acceptance criteria and manual outlier assessment). Each model used the linear equation outlined in Equation \@ref(eq:SER-eq).  
...
where *i* is an individual fish for a species-waterbody-year combination; these models were run on training data until all combinations were exhausted. 
...
For STAN models, all models were run with chains and cores set to 4. We ran the models for [Hg] datasets using the stan_glmer default settings [@R-rstanarm]. [As] were lower sample size, and so required higher iterations to achieve reliable model results, these were run with 9000 iterations and an adapt_delta setting of 0.99, this essentially makes acceptance criteria for posterior distributions during the adaptation period more strict, and lowers the step-size of the model, while increasing the computation time. The 50th percentile of predicted values were used to calculate residuals, RMSE~event~ and RMSE~global~ for these models.
...
INLA models were run in the R-INLA package, with precision of priors set to 0.001, and the formula structure was entered for the "iid2d" model to account for the covariance of the random effects due to waterbody [@R-INLA]. The 50th percentile of predicted values were used to calculate residuals, RMSE~event~ and RMSE~global~ for these models. 
...
predicted_data <- predicted_data %>% group_by(SAMPLE_YEAR, SPECIES_NAME, WATERBODY_CODE, CONTAMINANT) %>% 
  ...
  mutate(method = factor(method, levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))) %>% 
  filter(!N_fish == 0)
...
pred.plot <- predicted_data %>% pivot_wider(id_cols = c("N_fish", "SAMPLE_YEAR", "SPECIES_NAME", "WATERBODY_CODE", "CONTAMINANT", "WEIGHT_GRAM"), names_from = method, values_from = pred.fit, values_fn = mean) %>%
  pivot_longer(cols = c(MCMC, INLA, LMER, boot_LMER), values_to = "pred.fit", names_to = "method")%>%
  mutate(method = factor(method, levels = c("LMER", "boot_LMER", "MCMC", "INLA")))
...
lm_sums <- pred.plot %>% 
  ...
  mutate(CONTAMINANT = str_extract(set, "^Hg|^As"), 
         SPECIES_NAME = str_extract(set, "Lake Trout|Walleye|Northern Pike"),
         method = factor(str_extract(set, "SER|boot_LMER|LMER|MCMC|INLA"), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA")),
...
Predictions from LMER and LMER-bootstrapped approaches were consistent with each other, and had a general association with predicted values from SER, but much more noise in the predictions than the MCMC or INLA approaches. The MCMC and INLA approaches both had predicted values that performed very comparably with SER predictions, with most values greater than 1 ug/g~wet weight~ falling within 25% of the predicted SER values. The values lower than 1 ug/g~wet weight~ had more variance than this, but stayed within approximately 0.5 ug/g~wet weight~(Fig \@ref(fig:predictions)). Additional graphing of these predictions revealed that much of the deviations in these predictions were from predictions of the 1000g weights, and the 500g weight predictions had stronger associations (Supplemental Figs \@ref(fig:pred-500g), \@ref(fig:pred-1000g)).
...
ggplot(models_RMSE %>% 
         mutate(response = factor(gsub("_RMSE", "", response), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))),
...
wilcox.sig <- models_accuracy_by_N %>%
         ...
         filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "MCMC_resid"))%>% 
         filter(N_fish >= 5) %>% 
         mutate(response = factor(gsub("_resid|_residuals", "", gsub("boot", "boot_LMER", response)), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))) %>%
  group_by(CONTAMINANT, SPECIES_NAME) %>%
...
ggplot(models_accuracy_by_N %>%
         ...
         filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "MCMC_resid"))%>% 
         filter(N_fish >= 5) %>% 
         mutate(response = factor(gsub("_resid|_residuals", "", gsub("boot", "boot_LMER", response)), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))), aes(weight_group, predicted_value, color = response)) + 
  geom_boxplot() +
  #geom_smooth() + 
...
ggplot(models_accuracy_by_N %>% 
         ...
         mutate(weight_group = ifelse(WEIGHT_GRAM >= 400 & WEIGHT_GRAM <= 600, "400 - 600", "900-1100")) %>%
         filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "MCMC_resid"))%>% 
         mutate(response = factor(gsub("_resid|_residuals", "", gsub("boot", "boot_LMER", response)), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))), aes(VALUE, predicted_value, color = response, shape = weight_group)) + 
...
```{r resids-percent-1000g, fig.width=8, fig.height=10, fig.cap = "Predicted concentration of [As] and [Hg] in fish tissue of 400-600g and 900-1100g fish compared to known values. Points display each individual fish prediction and a solid red line shows a 1-1 relationship. Dashed red lines provide an interval for predicted values that are within 25% of the measured value. Correlation statistics are included where there were enough values to compute them."}
## Lm summaries
lm_sums <- models_accuracy_by_N %>% 
  ...
  mutate(CONTAMINANT = str_extract(set, "^Hg|^As"), 
         SPECIES_NAME = str_extract(set, "Lake Trout|Walleye|Northern Pike"),
         response = factor(str_extract(set, "SER|boot_LMER|LMER|MCMC|INLA"), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA")),
         R2.text = paste0("cor = ", Cor, "\np ",ifelse(p < 0.05, "< 0.05", paste0("= ", p))))
...
plot1 <- ggplot(models_accuracy_by_N %>% 
         ...
         filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "MCMC_resid"))%>% 
         mutate(response = factor(gsub("_resid|_residuals", "", gsub("boot", "boot_LMER", response)), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))), aes(VALUE, (predicted_value+VALUE), color = response)) + 
  geom_point(alpha=0.25) +
  geom_abline(slope = 1, color = "red") +
  geom_abline(slope = 1.25, color = "red", lty = 2) +
  geom_abline(slope = 0.75, color = "red", lty = 2) +
...
Each modelling approach resulted in different error distributions, with all approaches estimating test data values with averaged accuracy for [As] falling close to to 25% accuracy at most concentrations, and good accuracy (average estimated prediction within 25% accuracy) for [Hg] models up to a concentration of approximately 1.25 ug/g~wet\ weight~. There were biases in observed error that appeared to be driven by test values higher than 1.25 ug/g~wet\ weight~, with SER, LMER and bootstrapped LMER values resulting in underestimated predictions, and MCMC and INLA resulting in overestimated values (Fig \@ref(fig:resids-percent-1000g)). 
...
ggplot(models_accuracy_by_N %>% 
         ...
         #filter(N_fish < 5) %>% 
         filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "MCMC_resid"))%>% 
         mutate(response = factor(gsub("_resid|_residuals", "", gsub("boot", "boot_LMER", response)), levels = c("SER", "LMER", "boot_LMER", "MCMC", "INLA"))), aes(round(VALUE, 1), (predicted_value+VALUE)/VALUE, color = response, group = as.factor(round(VALUE, 1)))) + 
  geom_boxplot() +
...
models_overallRMSE <- models_accuracy_by_N %>% 
  filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "MCMC_resid", "INLA_resid")) %>% 
...
The processed data and scripts used to create this manuscript are available for download at the public repository managed by the WETlab at the Great Lakes Forestry Centre:  [https://github.com/GLFC-WET/HGAS_master](https://github.com/GLFC-WET/HGAS_master)
...
```{r response-values, fig.height=6, fig.cap = "Number of fish with different contaminant concentrations"}
...
```{r weight-values, fig.height = 10, fig.height=6, fig.cap = "Number of fish with different contaminant concentrations. Vertical black lines show the weights that are commonly used for standardization."}
...
```{r pred-500g, fig.height = 10, fig.cap = "Predicted concentration of [As] and [Hg] in fish tissue of 500g fish compared to values predicted through the SER approach. Points display each individual fish prediction and a solid red line shows a 1-1 relationship. Dashed red lines provide an interval for predicted values that are within 25% of the predicted SER value. Correlation statistics are included in the top left of each panel."}
