...
ggplot(models_accuracy_by_N %>% filter((WEIGHT_GRAM >= 400 & WEIGHT_GRAM <= 600)|(WEIGHT_GRAM >= 900 & WEIGHT_GRAM <= 1100)) %>% filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "RSTAN_resid")), aes(WEIGHT_GRAM, predicted_value, color = response)) + 
...
```{r resids-small-eventsize}
## plot the accuracy by weight of fish
ggplot(models_accuracy_by_N %>% filter((WEIGHT_GRAM >= 400 & WEIGHT_GRAM <= 600)|(WEIGHT_GRAM >= 900 & WEIGHT_GRAM <= 1100)) %>% filter(N_fish < 5) %>% filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "RSTAN_resid")), aes(WEIGHT_GRAM, predicted_value, color = response)) + 
  geom_point(alpha = 0.2) +
  #geom_smooth() + 
  geom_vline(xintercept = c(500, 1000), color = "red") +
  ylab("Residual") +
...
ggplot(models_accuracy_by_N %>% filter((WEIGHT_GRAM >= 400 & WEIGHT_GRAM <= 600)|(WEIGHT_GRAM >= 900 & WEIGHT_GRAM <= 1100)) %>% filter(N_fish < 10) %>% filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "RSTAN_resid")), aes(VALUE, predicted_value, color = response)) + 
...
```{r resids-percent-1000g}
## plot the accuracy by the amount of contaminant being predicted
ggplot(models_accuracy_by_N %>% filter((WEIGHT_GRAM >= 900 & WEIGHT_GRAM <= 1100)) %>% filter(N_fish < 10) %>% filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "RSTAN_resid")), aes(VALUE, predicted_value, color = response)) + 
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = c(0.1, -0.1), color = "red") +
  ylab("Residual") +
  facet_grid(CONTAMINANT+SPECIES_NAME~response, scales = "free") + 
...
```{r resids-percent-500g}
## plot the accuracy by the amount of contaminant being predicted
ggplot(models_accuracy_by_N %>% filter((WEIGHT_GRAM >= 400 & WEIGHT_GRAM <= 600)) %>% filter(N_fish > 12) %>% filter(response %in% c("LMER_resid", "boot_resid", "SER_residuals", "INLA_resid", "RSTAN_resid")), aes(VALUE, predicted_value, color = response)) + 
  geom_point(alpha = 0.1) +
  geom_hline(yintercept = c(0.1, -0.1), color = "red") +
  ylab("Residual") +
  facet_grid(CONTAMINANT+SPECIES_NAME~response, scales = "free") + 
