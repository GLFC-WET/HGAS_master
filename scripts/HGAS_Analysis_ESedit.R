...
dotchart(log(hg_sub$WEIGHT_GRAM)) # few outliers and majority of data b/w 5 and 9 
...
dotchart(log(hg_sub$LENGTH_CM)) # few outliers and majority of data b/w 3 and 5
...
## Compile statistics about how many species 
hg_sampling_event_statistics_by_species <- hg
...
Hg_INLA_LT_MASS <- inla(VALUE_LOG ~ WEIGHT_GRAM_LOG + 
                       f(WATERBODY_CODE1, n = 2 * Hg_LT$n_waterbody, model = "iid2d") + ## This codes the waterbody as a two dimensional random effect
                       f(WATERBODY_CODE2, WEIGHT_GRAM_LOG, copy = "WATERBODY_CODE1") + ## This is to code the random effect of waterbody and weight
                       f(WATERBODY_CODE_SAMPLE_YEAR, model = "iid"), ## this codes the random effect of each sampling event
                     data = Hg_LT, 
...
As_sub <- subset(As_sub, Taxon %in% c("LT", "WALL", "NP"))
...
sum(As_sub$RWT>= 900 & As_sub$RWT<= 1100)
