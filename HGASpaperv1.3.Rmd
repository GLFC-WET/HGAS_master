...
for(i in 1:nrow(Multi.year)){
  ...
  for(num in 3:set$max_N_fish){
        ...
        set_count = 1 ## keep track of the 'replicate' set that the sample is
        ...
        for(lakes in (2:length(unique(subsamp_data$WATERBODY_CODE)))[2:length(unique(subsamp_data$WATERBODY_CODE)) %in% c(2,3,5,7,9,10,15,20,30,40,50,75,100,150,200,250,300,500,750,1000)]){
          ...
          lake_set <- c(lake.at.N, sample(unique(subsamp_data$WATERBODY_CODE[subsamp_data$WATERBODY_CODE != lake.at.N]), lakes-1))
            ...
            write_rds(sample_set, paste0("temp/", paste(set$SPECIES_NAME, set$CONTAMINANT, lakes, num, s, sep = "_"), ".rds"))
            n_samples <- c(n_samples,  paste0("temp/", paste(set$SPECIES_NAME, set$CONTAMINANT, lakes, num, s, sep = "_"), ".rds"))
...
if(!file.exists("out_workspaces/LMER_numfish_models.RDS")){
  ## Produce individual models for each contaminant/species combination
  LMER_fish_data <- lapply(n_samples, function(x){
    
    if(sum(!is.na(readRDS(x)$VALUE_LOG_TEST)) < 3){ 
      out = NULL 
    }else{temp <- (readRDS(x) %>% lmer_bootstrap_prep() %>% 
  ...
  LMER_fish_data <- LMER_fish_data %>% discard(~ is.null(.x)) %>% bind_rows(.id = "set")
  ## Cache this dataset, because TAKES FOREVER
...
keepfiles <- c(keepfiles, "INLA_fish_data")
rm(list = ls()[!ls() %in% keepfiles])
```
